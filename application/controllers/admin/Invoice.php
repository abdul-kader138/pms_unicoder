...
//        pdf_create($viewfile, 'Invoice# ' . $data['invoice_info']->reference_no);
		...
		$pdf->WriteHTML($viewfile, true, 0, true, 0);
		
		$filename = 'Invoice ' . $data['invoice_info']->reference_no;
		$pdf->Output("" . $filename . ".pdf", 'I');


		/*$this->dompdf->load_html($viewfile);
		$this->dompdf->render();
    ...
    public function save_invoice($id = NULL)
    {
		...
		if(!empty($data['client_id']))
		{
			...
			$data['invoice_date'] = date('Y-m-d H:i', strtotime($this->input->post('invoice_date', TRUE).' '. date('H:i')));
			...
			$data['due_date'] = date('Y-m-d H:i', strtotime($this->input->post('due_date', TRUE).' '. date('H:i')));
			...
			$payment_save = $this->input->post('payment_save');
			if (!empty($payment_save))
			{
				redirect('admin/invoice/manage_invoice/payment/' . $invoice_id);
			}
			else if($invoice_id)
			{
...
//            redirect($_SERVER['HTTP_REFERER']);
            ...
            if ($paid_amount != 0) {
                if ($paid_amount > $due) {
                    ...
                    $data = array(
                        ...
                        'amount' => $paid_amount,
                        'payment_date' => date('Y-m-d H:i', strtotime($this->input->post('payment_date', TRUE).' '. date('H:i'))),
                    ...
                    if ($this->input->post('save_into_account') == 'on') {
                        ...
                        if (!empty($account_id)) {
                            ...
                            $tr_data = array(
                                ...
                                'amount' => $paid_amount,
                                'credit' => $paid_amount,
                                'date' => date('Y-m-d H:i', strtotime($this->input->post('payment_date', TRUE).' '. date('H:i'))),
                                'paid_by' => $inv_info->client_id,
    ...
    public function send_payment($invoices_id, $paid_amount)
    {
    ...
    public function send_invoice_email($invoice_id, $row = null)
    {
        if (!empty($row)) {
    ...
    function send_email_invoice($invoice_id, $message, $subject)
    {
		...
		if(!empty($client_info) && !empty($client_info->email))
		{
