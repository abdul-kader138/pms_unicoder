...
class report extends Admin_Controller
{
    ...
    {
        $data['title'] = lang('report_by_day');

        if ($this->input->post('year', TRUE)) { // if input year 
            $data['year'] = $this->input->post('year', TRUE);
	        $data['current_day'] = date('D', strtotime($data['year']));
        } else { // else current year
			$data['current_day'] = date('D');
            $data['year'] = date('d-m-Y'); // get current year
        }
        // get all expense list by year and month
        $data['report_by_day'] = $this->get_report_by_day($data['year']);

        $data['subview'] = $this->load->view('admin/report/report_by_day', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    public function get_report_by_day($year)
    {// this function is to create get monthy recap report
        $start_date = $year;
        $get_expense_list = $this->report_model->get_report_by_date($start_date); // get all report by start date and in date 
        return $get_expense_list; // return the result
    }
    public function report_by_day_pdf($year)
    {
        $data['report_list'] = $this->get_report_by_day($year);
        $current_day = date('D', strtotime($year)); // get full name of month by date query                
        if ($current_day == "Sun") {
        	$month_name =  lang('Sunday');
		}elseif ($current_day == "Mon") {
			$month_name = lang('Monday');
        }elseif ($current_day == "Tue") {
        	$month_name = lang('Tuesday');
        }elseif ($current_day == "Wed") {
        	$month_name = lang('Wednesday');
        }elseif ($current_day == "Thu") {
        	$month_name = lang('Thursday');
        }else if ($current_day == "Fri") {
        	$month_name = lang('Friday');
        }elseif ($current_day == "Sat") {
        	$month_name = lang('Saturday');
        }
        $data['monthyaer'] = $month_name . '  ' . $year;
        $this->load->helper('dompdf');
        $viewfile = $this->load->view('admin/report/report_by_day_pdf', $data, TRUE);
        pdf_create($viewfile, lang('report_by_day') . '- ' . $data['monthyaer']);
    }
    ...
    function sales_report_pdf($filterBy = null, $status, $start_date = null, $end_date = null)
    {
        ...
        if (!empty($filterBy)) {
            ...
            if ($filterBy == 'invoices' || $filterBy == 'invoice_by_client') {
                ...
                $range = array($start_date, $end_date);
                $data['status'] = $status;
                $data['all_estimates'] = $this->estimates_modela->get_estimate_report($data['status'], $range);
