    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?= lang('add_client') ?></h4>
            </div>
            <div class="modal-body">
                <form role="form" enctype="multipart/form-data" id="form" data-parsley-validate="" novalidate=""
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-5">
                            <input type="text" class="form-control" value="<?php if (!empty($client_info->mobile)) { echo $client_info->mobile; } ?>" name="mobile">
                        </div>
                    </div>
					<div class="form-group">
                        <label class="col-lg-3 control-label"><?= lang('email') ?></label>
                        <div class="col-lg-5">
                            <input type="text" class="form-control" value="<?php if (!empty($client_info->email)) { echo $client_info->email; } ?>" name="email">
                        </div>
                    </div>
...
<!-- end modal-->
<form name="myform" role="form" enctype="multipart/form-data" id="myform" action="<?php echo base_url(); ?>admin/invoice/save_invoice/<?php
		if (!empty($invoice_info)) {
			echo $invoice_info->invoices_id;
		}
		?>" method="post" class="form-horizontal  ">
    <div class="<?php
					if (!isset($invoice_info) || (isset($invoice_info) && !empty($invoices_to_merge) && count($invoices_to_merge) == 0)) {
						echo ' hide';
					}
					?>" id="invoice_top_info">
        <div class="panel-body">
            <div class="row">
                <div id="merge" class="col-md-8">
					<?php
					if (isset($invoice_info) && !empty($invoices_to_merge)) {
...
if ($this->session->userdata('user_type') == 1) {
	...
	?>
		...
		<div class="btn-xs btn btn-white pull-left ml ">
			<a class="text-dark" id="change_report" href="<?= base_url() ?>admin/dashboard/change_report/<?= $url ?>"><i class="<?= $icon ?>"></i>
				<span class="hidden-xs"><?= ' ' . lang('quick_state') . ' ' . lang($url) . ' ' . lang('always') ?></span></a>
		</div>
	<?php
}
...
if (!empty($created) || !empty($edited)) { ?>
	<a data-toggle="modal" data-target="#myModal" href="<?= base_url() ?>admin/invoice/zipped/invoice"  class="btn btn-success btn-xs ml-lg"><?= lang('zip_invoice') ?></a>	
	        ...
	        <div class="col-sm-12">
	            <div class="nav-tabs-custom">
	                ...
	                <ul class="nav nav-tabs">
	                    <li class=""><a href="#manage" data-toggle="tab"><?= lang('all_invoices') ?></a></li>
	                    <li class="active"><a href="#create" data-toggle="tab"><?= lang('create_invoice') ?></a></li>
	                </ul>
									...
									<table class="table table-striped DataTables " id="DataTables" cellspacing="0" width="100%">
										<thead>
											<tr>
												...
												<th><?= lang('client_name') ?> - <?= lang('mobil') ?></th>
												<th class="col-currency"><?= lang('due_amount') ?></th>
												<th><?= lang('status') ?></th>
												<?php
												$show_custom_fields = custom_form_table(9, null);
											...
											if (!empty($all_invoices_info)) {
												...
												foreach ($all_invoices_info as $v_invoices) {
                                                    ...
                                                    <td><?= $k ?></td>
														...
														?>
														<td><?= $client_name; ?><?php if($client_info){ echo $client_info->mobile ? ' - '.$client_info->mobile : ""; }?></td>
														<?php
														...
														<?php if ($v_invoices->recurring == 'Yes') { ?>
															...
															<?php
															$show_custom_fields = custom_form_table(9, $v_invoices->invoices_id);
															if (!empty($show_custom_fields)) {
																foreach ($show_custom_fields as $c_label => $v_fields) {
																	if (!empty($c_label)) {
																		?>
				...
				<?php if (!empty($can_edit) && !empty($edited)) { ?>
					...
					<?= lang('change_status') ?>
																		...
																		<ul class="dropdown-menu animated zoomIn">
                                                                            ...
                                                                            <li>
                                                                                <a href="<?= base_url() ?>admin/sms/send_invoice_sms/<?= $v_invoices->invoices_id ?>" data-toggle="modal" data-target="#myModal" title="<?= lang('sms_invoice') ?>"><?= lang('sms_invoice') ?></a>
                                                                            </li>
                                                                            <li>
                                                                                <a href="<?= base_url() ?>admin/sms/send_invoice_sms/<?= $v_invoices->invoices_id ?>?type=reminder" data-toggle="modal" data-target="#myModal"
						...
						<div class="tab-pane active" style="padding: 10px" id="create">
							...
							<div class="row mb-lg invoice accounting-template">
								<div class="col-md-12 br pv" style="padding: 10px">
									<div class="row">
										<div class="form-group">
											<label class="col-sm-3 control-label"><?= lang('reference_no') ?> <span class="text-danger">*</span></label>
											<div class="col-sm-6">
									...
									<div class="f_client_id">
										<div class="form-group">
											...
											<div class="col-lg-5">
												<select class="form-control select_box" style="width: 100%" name="client_id" id="client_id" value="New Client" onchange="get_project_by_id(this.value)" required="required">
													<option value="new client" selected="selected"><?= lang('new_client')?></option>
													<!--                                                        <option value="">-->
													<? //= lang('select') . ' ' . lang('client') ?><!--</option>-->
															...
															if (!empty($all_client)) {
																foreach ($all_client as $v_client) {
                                                                    ...
                                                                    if(!empty($this->input->get('selected')) and $v_client->client_id == $this->input->get('selected')){
																		$selected = " selected";
																	}else{
																		$selected = "";
																	}
											...
											<!-- button open modal-->
											<div class="col-lg-2">
												<button type="button" data-toggle="modal" data-target="#Add_clint" class="btn btn-success"><?= lang('add_client') ?>
												</button>
											</div>
										</div>
										...
										<div class="col-lg-7">
											<select class="form-control" style="width: 100%"
													...
													id="client_project">
														...
														if(!empty($this->input->get('selected'))){
															if (!empty($project_info->project_id)) {
																$project_id = $project_info->project_id;
															} elseif ($invoice_info->project_id) {
																$project_id = $invoice_info->project_id;
															}
															$all_project = $this->db->where('client_id', $this->input->get('selected'))->get('tbl_project')->result();
...
';
									...
									<div class="form-group" style="display:none;">
										...
										<div class="col-sm-7">
											...
											<input type="hidden" name="discount_type" value="after_tax" />
										</div>
									</div>
					...
					?>
									...
									<input type="hidden" name="allow_payumoney" value="no" />
													<?php
														   $show_pay = 0;
														   if ($show_pay == 1) {
															   if (config_item('paypal_status') == 'active'):
																   ?>
											<div class="form-group">
												<label for="field-1"
													   class="col-sm-3 control-label"><?= lang('allow_paypal') ?></label>
												<div class="col-sm-7">
					...
					<?php endif ?>
										<?php if (config_item('stripe_status') == 'active'): ?>
											<div class="form-group">
												<label for="field-1"
													   class="col-sm-3 control-label"><?= lang('allow_stripe') ?></label>
												<div class="col-sm-7">
													<div class="checkbox c-checkbox">
														<label class="needsclick">
															<input type="checkbox" value="Yes"
					...
					<?php if (config_item('2checkout_status') == 'active'): ?>
											<div class="form-group">
												<label for="field-1"
													   class="col-sm-3 control-label"><?= lang('allow_2checkout') ?></label>
												...
												<div class="col-sm-7">
													<div class="checkbox c-checkbox">
														<label class="needsclick">
															<input type="checkbox" value="Yes"
					<?php
					if (!empty($invoice_info) && $invoice_info->allow_mollie == 'Yes') {
					echo 'checked';
					}
					?>
												...
												<div class="col-sm-7">
													<div class="checkbox c-checkbox">
														<label class="needsclick">
															<input type="checkbox" value="Yes"
											<?php
											if (!empty($invoice_info) && $invoice_info->allow_payumoney == 'Yes') {
												echo 'checked';
											}
											?>
	...
	<?php
	if (isset($invoice_info) && $invoice_info->show_quantity_as == 'qty') {
		echo 'checked';
	...
	?>>
											...
											<label class="radio-inline c-radio">
												<input type="radio" value="qty_hours"
													   ...
													   name="show_quantity_as"
														<?php
														if (isset($invoice_info) && $invoice_info->show_quantity_as == 'qty_hours' || isset($qty_hrs_quantity)) {
															echo 'checked';
														}
														?>>
												<span
									...
									<div class="col-xs-8 pull-right">
										<table class="table text-right">
											<tbody>
	                                            ...
	                                            <tr style="display:none;">
	                                                <td>
	                                                    <div class="row">
	                                                        ...
	                                                        <div class="col-md-5">
	                                                            <input type="text" data-parsley-type="number"
	                                                                   value="<?php echo isset($invoice_info) ? $invoice_info->adjustment : 0;?>" class="form-control pull-left"
	                                                                   name="adjustment">
	                                                        </div>
	                                                    </div>
	                                                </td>
	                                            ...
	                                            <tr>
	                                                <td><span class="bold"><?php echo lang('total'); ?> :</span> </td>
	                                                <td class="total"></td>
	                                            </tr>
											</tbody>
										</table>
	...
	?></textarea>
							...
							<div class="modal-footer">
								<input type="submit" value="Create Invoice & Payment" name="payment_save" class="btn btn-primary">
								<input type="submit" value="<?= lang('save_as_draft') ?>" name="save_as_draft" class="btn btn-primary">
								<input type="submit" value="<?= lang('new_in') ?>" name="update" class="btn btn-success">
							</div>
						</div>
					</div>
				</div>
